...
def scrape_amazon_data(amazonUrl, cookie, asin, bOnlyProductInformation = False):
    ...
    HEADERS = {
        ...
        # "Cookie": "session-id=131-8029421-3272325;session-id-time=2082787201l;i18n-prefs=USD;ubid-main=135-3117522-7101258;session-token=I5SsGD39vSJ0AR/7I08yDfRXXN7Xo8UpQUD6BWHOY+v+8vbgDe8uyMeccS7GTtIJT880t9HYVWdDne80iYqqV3baNx3h5PmzSRGXPyBSAiFs4f45va8FYm5W2AlnCLbylLp9x4tvBq/UzHoG5uvWd4556ijY2LJ1Sg+CgP+o87TcD/DLacuxaA/lGvxR9zitENmFpCyc8VW89YyPMdfCgymb/ShWPHc2NrlQk5N8m5s=;csm-hit=tb:51X052CNK7HPRVBMFDW0+s-PKE6AT0W4M2AAE5MZTTQ|1683513663915&t:1683513663915&adb:adblk_no;"
    }
    ...
    if bOnlyProductInformation == True:
        try:
            ...
            print( "Faild to get product information" )
            return {}, []
    ...
    def get_reviews(page_url):
        ...
        headers = { 'Authorization': 'Bearer 8ccc7f1b-c925-4ff2-a147-bd4aa8a994d8', 'Content-Type': 'text/plain' }

        data = { "input": f"I have provided link to my Amazon product reviews and you find all reviews in amazon.com. Please read all reviews (from my product)and identify any patterns, trends, or significant differences in the responses. my product :[{page_url}] Give me a list of only text for reviews( span,!i !b data-hook: review-body, that is html tag of review content) about this pages." }
        req = requests.post('https://api.bardapi.dev/chat', headers=headers, json=data)
        data = json.loads(req.text)['output'].split("\n")
        for item in data:
    ...
    with ThreadPoolExecutor() as executor:
        page_num=5 # math.ceil(float(reviews_count.replace(",",""))/10)
        pages = [f"https://www.amazon.com/product-reviews/{asin}/ref=cm_cr_getr_d_paging_btm_next_3?ie=UTF8&reviewerType=all_reviews&sortBy=recent&pageNumber{i+1}&pageNumber={i+1}" for i in range(page_num)]
